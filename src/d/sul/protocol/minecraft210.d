...
static struct Packets {
	...
	static struct Play {
		...
		static struct PlayerBlockPlacement {
			...
			public uint face;
			public uint hand;
			public Tuple!(ubyte, "x", ubyte, "y", ubyte, "z") cursor;
			...
			public ubyte[] encode(bool write_id=true)() {
				...
				buffer~=varuint.encode(hand);
				buffer.length+=ubyte.sizeof; write!(ubyte, Endian.bigEndian)(buffer, cursor.x, buffer.length-ubyte.sizeof);buffer.length+=ubyte.sizeof; write!(ubyte, Endian.bigEndian)(buffer, cursor.y, buffer.length-ubyte.sizeof);buffer.length+=ubyte.sizeof; write!(ubyte, Endian.bigEndian)(buffer, cursor.z, buffer.length-ubyte.sizeof);
				return buffer;
			}
			...
			public typeof(this) decode(bool read_id=true)(ubyte[] buffer) {
				...
				hand=varuint.fromBuffer(buffer);
				if(buffer.length>=ubyte.sizeof){ cursor.x=read!(ubyte, Endian.bigEndian)(buffer); }if(buffer.length>=ubyte.sizeof){ cursor.y=read!(ubyte, Endian.bigEndian)(buffer); }if(buffer.length>=ubyte.sizeof){ cursor.z=read!(ubyte, Endian.bigEndian)(buffer); }
				return this;
			}
