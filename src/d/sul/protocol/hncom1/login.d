...
class ConnectionResponse : Buffer {
	...
	public override string toString() {
		return "ConnectionResponse(protocol: " ~ std.conv.to!string(this.protocol) ~ ", status: " ~ std.conv.to!string(this.status) ~ ")";
	}
...
class HubInfo : Buffer {
	...
	/**
	 * First number of the 4.294.967.296 reserved by the hub to compose node's UUIDs. Every
	 * UUID generated by the node is formed by the server's id (most signicant) and the
	 * next reserved uuid (least significant). This way every UUID in the hub and in the
	 * connected nodes is always different.
	 */
	...
	public override string toString() {
		return "HubInfo(time: " ~ std.conv.to!string(this.time) ~ ", serverId: " ~ std.conv.to!string(this.serverId) ~ ", reservedUuids: " ~ std.conv.to!string(this.reservedUuids) ~ ", displayName: " ~ std.conv.to!string(this.displayName) ~ ", onlineMode: " ~ std.conv.to!string(this.onlineMode) ~ ", gamesInfo: " ~ std.conv.to!string(this.gamesInfo) ~ ", online: " ~ std.conv.to!string(this.online) ~ ", max: " ~ std.conv.to!string(this.max) ~ ", language: " ~ std.conv.to!string(this.language) ~ ", acceptedLanguages: " ~ std.conv.to!string(this.acceptedLanguages) ~ ", socialJson: " ~ std.conv.to!string(this.socialJson) ~ ", additionalJson: " ~ std.conv.to!string(this.additionalJson) ~ ")";
	}
