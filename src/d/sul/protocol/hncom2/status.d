...
class AddNode : Buffer {
	...
	public pure nothrow @safe @nogc this(uint hubId, string name=string.init, bool main=bool.init, sul.protocol.hncom2.types.Game[] acceptedGames=(sul.protocol.hncom2.types.Game[]).init) {
		this.hubId = hubId;
		this.name = name;
		this.main = main;
...
class Log : Buffer {
	...
	// world
	public enum int NO_WORLD = -1;

	public enum string[] FIELDS = ["timestamp", "world", "logger", "message", "commandId"];
	...
	public pure nothrow @safe @nogc this(ulong timestamp, int world=int.init, string logger=string.init, string message=string.init, int commandId=int.init) {
	...
	public pure nothrow @safe ubyte[] encode(bool writeId=true)() {
		...
		writeBytes(varint.encode(world));
		if(world<0){ writeBytes(varuint.encode(cast(uint)logger.length)); writeString(logger); }
	...
	public pure nothrow @safe void decode(bool readId=true)() {
		...
		world=varint.decode(_buffer, &_index);
		if(world<0){ uint b9zv=varuint.decode(_buffer, &_index); logger=readString(b9zv); }
	...
	public override string toString() {
		return "Log(timestamp: " ~ std.conv.to!string(this.timestamp) ~ ", world: " ~ std.conv.to!string(this.world) ~ ", logger: " ~ std.conv.to!string(this.logger) ~ ", message: " ~ std.conv.to!string(this.message) ~ ", commandId: " ~ std.conv.to!string(this.commandId) ~ ")";
	}
...
class RemoteCommand : Buffer {
	...
	public pure nothrow @safe @nogc this(ubyte origin, sul.protocol.hncom2.types.Address sender=sul.protocol.hncom2.types.Address.init, string command=string.init, int commandId=int.init) {
		this.origin = origin;
		this.sender = sender;
...
class Reload : Buffer {
	...
	public pure nothrow @safe @nogc this(string displayName, sul.protocol.hncom2.types.Motd[] motds=(sul.protocol.hncom2.types.Motd[]).init, string language=string.init, string[] acceptedLanguages=(string[]).init, string socialJson=string.init) {
	...
	public pure nothrow @safe ubyte[] encode(bool writeId=true)() {
		...
		writeBytes(varuint.encode(cast(uint)motds.length)); foreach(b9zm;motds){ b9zm.encode(bufferInstance); }
	...
	public pure nothrow @safe void decode(bool readId=true)() {
		...
		motds.length=varuint.decode(_buffer, &_index); foreach(ref b9zm;motds){ b9zm.decode(bufferInstance); }
		...
		uint c9afsnb=varuint.decode(_buffer, &_index); socialJson=readString(c9afsnb);
	}
	...
	public override string toString() {
		return "Reload(displayName: " ~ std.conv.to!string(this.displayName) ~ ", motds: " ~ std.conv.to!string(this.motds) ~ ", language: " ~ std.conv.to!string(this.language) ~ ", acceptedLanguages: " ~ std.conv.to!string(this.acceptedLanguages) ~ ", socialJson: " ~ std.conv.to!string(this.socialJson) ~ ")";
	}
