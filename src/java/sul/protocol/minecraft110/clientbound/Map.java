...
public class Map extends Packet {
	...
	@Override
	public int length() {
		int length=Buffer.varuintLength(ID) + Buffer.varuintLength(mapId) + Buffer.varuintLength(icons.length) + Buffer.varuintLength(data.length) + data.length + 6; for(sul.protocol.minecraft110.types.Icon awnvbnm:icons){ length+=awnvbnm.length(); } return length;
	}
	...
	public byte[] encode() {
		...
		this.writeVaruint((int)icons.length); for(sul.protocol.minecraft110.types.Icon awnvbnm:icons){ this.writeBytes(awnvbnm.encode()); }
