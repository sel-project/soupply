...
public class MultiBlockChange extends Packet {
	...
	@Override
	public int length() {
		int length=Buffer.varuintLength(ID) + Buffer.varuintLength(changes.length) + 8; for(sul.protocol.minecraft210.types.BlockChange y2hhbmdlcw:changes){ length+=y2hhbmdlcw.length(); } return length;
	}
	...
	public byte[] encode() {
		...
		this.writeVaruint((int)changes.length); for(sul.protocol.minecraft210.types.BlockChange y2hhbmdlcw:changes){ this.writeBytes(y2hhbmdlcw.encode()); }
		return this._buffer;
	}
