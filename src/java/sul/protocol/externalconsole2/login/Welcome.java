...
public class Welcome extends Packet {
	...
	public int getId() {
		...
		@Override
		public int length() {
			int length=software.getBytes(StandardCharsets.UTF_8).length + versions.length + displayName.getBytes(StandardCharsets.UTF_8).length + connectedNodes.length*2 + 9; for(sul.protocol.externalconsole2.types.Game z2ftzxm:games){ length+=z2ftzxm.length(); };for(String y29ubmvjdgvktm9k:connectedNodes){ length+=y29ubmvjdgvktm9k.getBytes(StandardCharsets.UTF_8).length; } return length;
		}
		...
		public byte[] encode() {
			...
			this.writeBigEndianShort((short)games.length); for(sul.protocol.externalconsole2.types.Game z2ftzxm:games){ this.writeBytes(z2ftzxm.encode()); }
		...
		public void decode(byte[] buffer) {
			...
			int bgdhbwvz=readBigEndianShort(); games=new sul.protocol.externalconsole2.types.Game[bgdhbwvz]; for(int z2ftzxm=0;z2ftzxm<games.length;z2ftzxm++){ games[z2ftzxm]=new sul.protocol.externalconsole2.types.Game(); games[z2ftzxm]._index=this._index; games[z2ftzxm].decode(this._buffer); this._index=games[z2ftzxm]._index; }
