...
public class ListAddPlayer extends Packet {
	...
	@Override
	public int length() {
		int length=Buffer.varuintLength(name.getBytes(StandardCharsets.UTF_8).length) + name.getBytes(StandardCharsets.UTF_8).length + Buffer.varuintLength(properties.length) + Buffer.varuintLength(gamemode) + Buffer.varuintLength(latency) + Buffer.varuintLength(displayName.getBytes(StandardCharsets.UTF_8).length) + displayName.getBytes(StandardCharsets.UTF_8).length + 17; for(sul.protocol.minecraft109.types.Property chjvcgvydgllcw:properties){ length+=chjvcgvydgllcw.length(); } return length;
	}
	...
	public byte[] encode() {
		...
		this.writeVaruint((int)properties.length); for(sul.protocol.minecraft109.types.Property chjvcgvydgllcw:properties){ this.writeBytes(chjvcgvydgllcw.encode()); }
	...
	public void decode(byte[] buffer) {
		...
		int bhbyb3blcnrpzxm=this.readVaruint(); properties=new sul.protocol.minecraft109.types.Property[bhbyb3blcnrpzxm]; for(int chjvcgvydgllcw=0;chjvcgvydgllcw<properties.length;chjvcgvydgllcw++){ properties[chjvcgvydgllcw]=new sul.protocol.minecraft109.types.Property(); properties[chjvcgvydgllcw]._index=this._index; properties[chjvcgvydgllcw].decode(this._buffer); this._index=properties[chjvcgvydgllcw]._index; }
