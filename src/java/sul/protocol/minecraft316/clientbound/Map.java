...
public class Map extends Packet {
	...
	@Override
	public int length() {
		int length=Buffer.varuintLength(ID) + Buffer.varuintLength(mapId) + Buffer.varuintLength(icons.length) + Buffer.varuintLength(data.length) + data.length + 6; for(sul.protocol.minecraft316.types.Icon anbm:icons){ length+=anbm.length(); } return length;
	}
