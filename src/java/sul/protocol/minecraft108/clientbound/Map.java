...
public class Map extends Packet {
	...
	@Override
	public int length() {
		int length=Buffer.varuintLength(ID) + Buffer.varuintLength(mapId) + Buffer.varuintLength(icons.length) + Buffer.varuintLength(data.length) + data.length + 6; for(sul.protocol.minecraft108.types.Icon awnvbnm:icons){ length+=awnvbnm.length(); } return length;
	}
	...
	public byte[] encode() {
		...
		this.writeVaruint((int)icons.length); for(sul.protocol.minecraft108.types.Icon awnvbnm:icons){ this.writeBytes(awnvbnm.encode()); }
	...
	public void decode(byte[] buffer) {
		...
		int bgljb25z=this.readVaruint(); icons=new sul.protocol.minecraft108.types.Icon[bgljb25z]; for(int awnvbnm=0;awnvbnm<icons.length;awnvbnm++){ icons[awnvbnm]=new sul.protocol.minecraft108.types.Icon(); icons[awnvbnm]._index=this._index; icons[awnvbnm].decode(this._buffer); this._index=icons[awnvbnm]._index; }
